...
- Claude workflow auth: `pilot.yml`, `code-review.yml`, `issue-intake.yml`, and `revise.yml` all authenticate via `CLAUDE_CODE_OAUTH_TOKEN` (routes through Blake's Claude Max subscription). Generate the token locally with `claude setup-token`, then store as a repo secret. The `ANTHROPIC_API_KEY` secret is retained as a rollback path but no longer referenced in the workflows.
