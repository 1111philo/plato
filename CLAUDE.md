...
- Claude workflow auth: `pilot.yml`, `code-review.yml`, `issue-intake.yml`, and `revise.yml` all pass both `claude_code_oauth_token: ${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}` and `anthropic_api_key: ${{ secrets.ANTHROPIC_API_KEY }}` to `anthropics/claude-code-action@v1`. OAuth-only was attempted (PR #93) but the Max OAuth token returned `401 "Invalid bearer token"` from Anthropic's API on first real use (PR #94). The API-key fallback was restored; investigate the OAuth scope issue (Max tokens may only work via the Claude Code proxy, not direct API) before attempting OAuth-only again.
