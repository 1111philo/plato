...
- Courses: optional taxonomy for grouping lessons. Each course is a first-class `_system:course:<id>` sync-data record with `{ name, description }`. Each lesson carries an optional `course` field (course ID, like `userGroup` on users). Pure organization — courses don't carry visibility/ACL; lesson visibility is unchanged. The learner-facing `GET /v1/lessons` and `/v1/lessons/:id` endpoints inline `course: { id, name, description } | null` so the coach context (`buildContext` in `client/src/lib/lessonEngine.js`) can include `course: { name, description }` when assigned. Admin manages courses at `/plato/courses` and assigns via a dropdown in the lesson editor — assignments are persisted immediately on change. Course delete cascades: any lesson whose `course` field referenced the deleted course is rewritten with `course: null`.
...
- Admin nav order: Home, Lessons, Users, Courses, Customizer, Plugins
