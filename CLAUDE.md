...
- Auto code review: `.github/workflows/code-review.yml` runs on every PR (opened + synchronize) and posts a review as `claude[bot]`. To avoid a review-loop when `revise.yml` pushes a fixup on a `plato-pilot` PR, the workflow's guard step inspects the head commit's author email and skips the review steps when the author is `plato-pilot@users.noreply.github.com` — the job still completes so the required `review` status check is satisfied. Only one automated reviewer should be active at a time; if the hosted Claude GitHub App (github.com/apps/claude) is also installed with auto-review enabled, disable it there to prevent duplicate reviews.
