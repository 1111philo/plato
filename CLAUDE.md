...
- Issue intake: `.github/workflows/issue-intake.yml` runs on every newly opened issue and classifies it as either `ready-for-pilot` (enough detail for plato-pilot or a human to act on) or `needs-info` (posts ≤3 focused clarifying questions for the reporter). Non-bug issues get no label, no comment. The pilot workflow's triage step prefers `ready-for-pilot` issues over raw ones. Idempotent via the labels — the agent won't re-engage an issue it already labeled.
