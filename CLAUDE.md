...
- **9 AI agents** via Bedrock or Anthropic API (prompts in `client/prompts/`, each with an HTML header documenting reads/callers/purpose): coach, lesson-creator, lesson-owner, lesson-extractor, knowledge-base-editor, knowledge-base-extractor, learner-profile-owner, learner-profile-update, course-progress-update. Program KB + Lesson Catalog are appended to system prompts at runtime (`client/js/orchestrator.js`). That repeated prefix is prompt-cached (`server/src/lib/prompt-cache.js`) — **only for Claude models, and only above the 4096-token minimum**, below which Bedrock silently ignores `cache_control`. → [details](docs/ARCHITECTURE.md#prompt-caching)
...
- `server/src/lib/prompt-cache.js` — marks the repeated system-prompt prefix cacheable
