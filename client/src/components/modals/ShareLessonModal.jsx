  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setUsers(Array.isArray(data) ? data.filter(u => u.role !== 'admin') : []);
      } catch { /* ignore */ }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose which learners can access <strong>{lessonName}</strong>.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={selected.size === 0}>
            Share with {selected.size} {selected.size === 1 ? 'user' : 'users'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
