...
When you modify files in `client/prompts/` or `client/data/`, existing plato installs will surface these changes to admins as pending content updates on their dashboard. The system works by storing a SHA-256 hash (`bundledHash`) of each file's content in the database at seed time. On each admin dashboard load, the server compares bundled files on disk against the stored hashes and alerts admins to any differences.

Admins can:
- **Accept** — replace the database content with the new bundled version
- **Dismiss** — keep their current content and suppress the alert (until the file changes again)

This means admins who have customized their prompts won't have changes silently overwritten, but they'll always be aware of upstream improvements. When contributing prompt or content changes, test them with a real AI connection before submitting.
...
plato is a [microlearning](https://philosophers.group/platos-microlearning/) platform. Courses are designed to be completable in ~11 exchanges (~20 minutes). This means:
