...
Prompts, courses, and the knowledge base are stored in the database and editable by admins at `/plato-admin`. The source markdown files in `client/prompts/` and `client/data/` serve as seed data — run `server/scripts/seed-content.js` to populate or reset them.
...
1. Create a markdown file following the format:
   ...
   - Third objective
   ```
2. Add it via the admin dashboard at `/plato-admin/courses`, or place it in `client/data/courses/` and re-run the seed script.
