...
- **Changed** `DELETE /v1/sync` (the learner's "reset my own data" endpoint) preserves `userMeta:*` records — they're admin-owned. With this change, every learner-facing `/v1/sync` path excludes plugin-owned data: GET (bulk + single), PUT (single), DELETE (single + bulk). `userMeta:*` is cleaned only via account-deletion or a plugin's own `onUninstall`.
